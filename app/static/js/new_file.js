...
var promise = $.ajax({type: "GET", url: "/api/categories"});

promise.done(function(data) {
  data.forEach(function(item) {
    groups_list.push(item.name);
  });
});
...
function addHeatmap(markersData) {
    removeHeatmap(); // Удаляем все текущие тепловые точки

    var heatMapData = markersData.map(function(marker) {
        var weight = marker.likeness; // Здесь может потребоваться изменить, в зависимости от вашей логики
        var coordinates = [parseFloat(marker.coord_y), parseFloat(marker.coord_x)];
        return coordinates.concat(weight);
    });
 ...
 for (var j = 0; j < heatMapData.length; j++) {
        ...
        // Определяем градиент цветов в зависимости от веса точки
        if (weight >= 10) {
            heatmapGradient = {
                0.1: colors[0]
            };
        ...
        // Создаем тепловую точку и добавляем ее на карту
...
myMap.options.set('fullscreenZIndex', 1);
    ...
    // Создадим 7 пунктов выпадающего списка.
       ...
       myMap.controls.add(listBoxControl, { float: 'none', position: { right: 10, top: 45 } });
listBoxControl.events.add(['select', 'deselect'], function(e) {
    var listBoxItem = e.get('target');
    var filters = ymaps.util.extend({}, listBoxControl.state.get('filters'));
    filters[listBoxItem.data.get('content')] = listBoxItem.isSelected();
    listBoxControl.state.set('filters', filters);
    // Проверяем, определены ли фильтры
    if (filters) {
        addMarkers(filters);
    }
});
...
function addMarkers(filters) {
    console.log('addMarkers вызвана с фильтрами:', filters); // Добавляем эту строку
    const trueFilterKeys = Object.keys(filters).filter(key => filters[key] === true);
    const selectedIndexArray = trueFilterKeys.map(key => groups_list.indexOf(key) + 1);
    const categoriesIdString = selectedIndexArray.join(',');
$.ajax({
    url: '/api/markers?categories_id=[' + categoriesIdString + ']',
    type: 'GET',
    success: function(data) {
        console.log('Данные о маркерах успешно загружены:', data); // Добавляем эту строку
        if (data.length > 0) {
            addHeatmap(data); // Добавляем тепловые точки только если есть данные
        }else{
            removeHeatmap();
        }
    },
    error: function(error) {
        console.error('Ошибка при получении данных о маркерах:', error);
    }
});
}
...
// Получаем список групп из Flask API и создаем для каждой группы элемент ListBoxItem
$.ajax({
    url: '/api/categories',
    type: 'GET',
    success: function(response) {
        var groups = response; // Предположим, что данные содержат список ваших групп
        console.log(response);
        if (groups && groups.length > 0) {
            groups.forEach(function(group) {
                var item = new ymaps.control.ListBoxItem({
                    data: { content: group.name } // Предполагается, что 'name' содержит имя группы
                });

                // Добавляем обработчик события клика для каждого элемента ListBoxItem
                item.events.add('click', function(event) {
                    var target = event.get('target');
                    var groupName = target.data.get('content'); // Получаем название группы
                    if (groupName) {
                        var isChecked = target.isSelected();
                        if (!isChecked) {
                            addHeatmap(groupName); // Передаем название группы в функцию добавления тепловых точек
                        } else {
                            removeHeatmap();
                        }
                    }
                ...
                // Добавляем элемент ListBoxItem в элемент ListBox
                listBoxWithCheckbox.add(item);
            });
        } else {
